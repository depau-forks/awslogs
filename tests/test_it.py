...
class TestAWSLogsDatetimeParse(unittest.TestCase):
    ...
    def test_parse_datetime(self, datetime_mock, botoclient):
        ...
        for expected_iso, dateutil_time in plan:
            self.assertEqual(
                awslogs.parse_datetime(dateutil_time), iso2epoch(expected_iso)
            )
...
class TestAWSLogs(unittest.TestCase):
    ...
    def test_get_groups(self, botoclient):
        ...
        awslogs = AWSLogs()
        self.assertEqual(
            [g for g in awslogs.get_groups()], ["A", "B", "C", "D", "E", "F", "G"]
        )
    ...
    def test_get_streams(self, botoclient):
        ...
        client.get_paginator.return_value.paginate.return_value = [
            {
                "logStreams": [self._stream("A"), self._stream("B"), self._stream("C")],
                "nextToken": 1,
            },
            {
                "logStreams": [self._stream("D"), self._stream("E"), self._stream("F")],
                "nextToken": 2,
            },
        ...
        self.assertEqual(
            [g for g in awslogs.get_streams()], ["A", "B", "C", "D", "E", "F", "G"]
        )
    ...
    def test_main_get_with_color(self, mock_stdout, botoclient):
        self.set_ABCDE_logs(botoclient)
        exit_code = main("awslogs get AAA DDD --color=always".split())
        output = mock_stdout.getvalue()
        expected = (
    ...
    def test_main_get_query(self, mock_stdout, botoclient):
        ...
        exit_code = main("awslogs get AAA DDD --query foo --color=always".split())
        output = mock_stdout.getvalue()
        expected = (
    ...
    def test_main_get_no_matching_streams(self, mock_stderr, botoclient):
        ...
        self.assertEqual(
            ...
            colored(
                "No streams match your pattern 'foo.*' " "for the given time period.\n",
                "red",
            ),
        )
        assert exit_code == 7
